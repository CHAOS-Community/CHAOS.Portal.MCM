...
namespace Chaos.Mcm.Extension.v6
{
  public class File : AMcmExtension
  {
    ...
    public File(IPortalApplication portalApplication, IMcmRepository mcmRepository, IPermissionManager permissionManager)
    ...
    public Data.Dto.File Create(Guid objectGuid, uint? parentFileID, uint formatID, uint destinationID, string filename,
                                string originalFilename, string folderPath)
    {
      ...
      var id = McmRepository.FileCreate(objectGuid, parentFileID, destinationID, filename, originalFilename, folderPath,
                                        formatID);
      var result = McmRepository.FileGet(id);

      ViewManager.Index(McmRepository.ObjectGet(objectGuid, true, true, true ,true, true));
    ...
    public ScalarResult Delete(uint id)
    {
      ...
      ViewManager.Index(McmRepository.ObjectGet(file.ObjectGuid, true, true, true, true, true));
