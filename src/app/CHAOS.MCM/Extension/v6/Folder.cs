...
namespace Chaos.Mcm.Extension.v6
{
  public class Folder : AMcmExtension
  {
    ...
    public FolderPermission GetPermission(uint folderID)
    {
      ...
      return new FolderPermission(userPermissions, groupPermissions) {FolderID = folderID};
    }
    ...
    public ScalarResult SetPermission(Guid? userGuid, Guid? groupGuid, uint folderID, uint permission)
    {
      ...
      if (!userGuid.HasValue || userGuid.Value != Request.User.Guid)
      {
        var folder = PermissionManager.GetFolders(folderID);

        // REVIEW: What permissions are required to remove a permission?
        if (
          !folder.DoesUserOrGroupHavePermission(Request.User.Guid, Request.Groups.Select(item => item.Guid),
                                                permission == 0
                                                  ? Permission.FolderPermission.Max
    ...
    public IEnumerable<IFolderInfo> Get(uint? id, uint? folderTypeID, uint? parentID, uint? permission)
    {
      ...
      var permissionEnum = (Permission.FolderPermission) (permission ?? (uint) Permission.FolderPermission.Read) |
      ...
      if (!parentID.HasValue && !id.HasValue)
        ...
        folderResults =
          PermissionManager.GetFolders(parentID.Value)
                           .GetSubFolders()
                           .Where(item => item.DoesUserOrGroupHavePermission(userGuid, groupGuids, permissionEnum));
      else
        folderResults =
          new[] {PermissionManager.GetFolders(id.Value)}.Where(
            item => item.DoesUserOrGroupHavePermission(userGuid, groupGuids, permissionEnum));
    ...
    public ScalarResult Delete(uint id)
    {
      ...
      if (
        !PermissionManager.GetFolders(id)
                          .DoesUserOrGroupHavePermission(userGuid, groupGuids, Permission.FolderPermission.Delete))
    ...
    public ScalarResult Update(uint id, string newTitle, uint? newFolderTypeID, uint? newParentID)
    {
      ...
      if (
        !PermissionManager.GetFolders(id)
                          .DoesUserOrGroupHavePermission(user.Guid, groups.Select(item => item.Guid),
                                                         Permission.FolderPermission.Update))
        throw new InsufficientPermissionsException("User does not have permission to give the requested permissions");
    ...
    public IFolderInfo Create(Guid? subscriptionGuid, string title, uint? parentID, uint folderTypeID)
    {
      ...
      if (subscription != null && subscription.Permission != SubscriptionPermission.CreateFolder)
        throw new InsufficientPermissionsException(
          "User does not have permission to create topfolders with the subscriptionGuid");

      if (parentID.HasValue &&
          !PermissionManager.GetFolders((uint) parentID)
                            .DoesUserOrGroupHavePermission(userGuid, groupGuids, Permission.FolderPermission.Write))
        throw new InsufficientPermissionsException("User does not have permission to create subfolders");

      var result = McmRepository.FolderCreate(userGuid, subscription == null ? (Guid?) null : subscription.Guid, title,
                                              parentID, folderTypeID);
