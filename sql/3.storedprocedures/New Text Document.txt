...
IF( IncludeFiles = 1 ) THEN
    ...
    FROM
        Object_Object_Join
        JOIN FileInfo ON Object.GUID = FileInfo.ObjectGUID
    WHERE
        Object_Object_Join.Object1GUID = RelatedObjectGUID
        AND (ObjectRelationTypeID IS NULL OR Object_Object_Join.ObjectRelationTypeID = ObjectRelationTypeID);
END IF;
...
IF( IncludeFolders = 1 ) THEN
    ...
    FROM
        Object_Object_Join
        JOIN Object_Folder_Join ON Object_Object_Join.Object2GUID = Object_Folder_Join.ObjectGUID
        JOIN Folder ON Object_Folder_Join.FolderID = Folder.ID
    WHERE
        Object_Object_Join.Object1GUID = RelatedObjectGUID
        AND (ObjectRelationTypeID IS NULL OR Object_Object_Join.ObjectRelationTypeID = ObjectRelationTypeID);
END IF;
...
IF( IncludeAccessPoints = 1 ) THEN
    ...
    FROM
        ...
        JOIN AccessPoint ON AccessPoint_Object_Join.AccessPointGUID = AccessPoint.GUID
    WHERE
        Object_Object_Join.Object1GUID = RelatedObjectGUID
        AND (ObjectRelationTypeID IS NULL OR Object_Object_Join.ObjectRelationTypeID = ObjectRelationTypeID);
END IF;
